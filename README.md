...
| Cloudflare Worker | Verifies Turnstile token, normalises email, writes to Supabase | Deployed to Cloudflare — source not in this repo |
| Worker config | Deployment settings, non-secret vars | `wrangler.toml` (Cloudflare dashboard) |
...
|---|---|---|
| `POST` | `https://safetynet-signup.campbell-mccord.workers.dev/signup` | Only accepted method |
...
| `js/utils.js` | `submitToWaitlist()`, Turnstile execution, honeypot checks |
...
| R7 | The README is not kept in sync with the source file. | Operators may use stale information. | Update both `README.md` and `safetynet-README_1827.020826.md` together. |
...
| Welcome email not received | Check Supabase dashboard → Database → Webhooks and Supabase edge function logs. | That the signup itself failed. |
...
7. Update both `README.md` and `safetynet-README_1827.020826.md`.
8. Update the timestamp in the header table and metadata block.
...
- The repository, architecture, deployment, or dependency evidence changes.
...
| Completeness | Partial | Usage, limitations, sources, and maintenance are covered; lockfile and version pinning not included. |
| Accuracy | Partial | Statements are traceable to supplied briefs and status-labelled; stack and data flow now confirmed. |
| Usability | Pass | Visitor and operator workflows, examples, troubleshooting, and update instructions are sequential. |

Verdict under `README-STD-001`: **Functional**.
...
― Location ―
Current Filepath: safetynet-landing/README.md
Source File:      safetynet-landing/safetynet-README_1827.020826.md
